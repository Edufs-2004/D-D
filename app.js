...
// 2. VARIABLES DE JUEGO (ESTADO GLOBAL)
...
function cargarListaPersonajes() {
    const guardados = JSON.parse(localStorage.getItem('dnd_personajes')) || [];
...
function guardarPersonaje() {
    ...
    if (!nombreStr) {
        alert("¡Ponle un nombre a tu personaje antes de guardarlo!");
        return;
    }

    if (!idPersonajeActual) idPersonajeActual = Date.now().toString();

    const personajeData = {
        id: idPersonajeActual,
        identidad: {
            nombre: nombreStr,
            imgUrl: document.getElementById('char-img-url').value,
            sexo: document.getElementById('char-sexo').value,
            edad: document.getElementById('char-edad').value,
            clase: document.getElementById('char-clase').value,
            historia: document.getElementById('char-historia').value
        },
        progreso: {
            modoJuego: modoJuego,
            puntosInicialesDisp: puntosInicialesDisp,
            nivelPersonaje: nivelPersonaje,
            puntosMejoraDisp: puntosMejoraDisp,
            hpActual: hpActual,
            manaActual: manaActual
        },
        statsArray: stats,
        efectosArray: efectosPersonaje,
        inventario: {
    ...
    for(let i=1; i<=5; i++) {
        personajeData.inventario.mochila.push({
            val: document.getElementById(`inv-${i}`).value,
            id: document.getElementById(`inv-${i}`).dataset.itemId || ""
        });
    }

    let guardados = JSON.parse(localStorage.getItem('dnd_personajes')) || [];
    const index = guardados.findIndex(p => p.id === idPersonajeActual);
    
    if (index > -1) guardados[index] = personajeData; 
    else guardados.push(personajeData); 

    localStorage.setItem('dnd_personajes', JSON.stringify(guardados));
    cargarListaPersonajes();
    alert("¡Personaje Guardado Correctamente!");
}

function cargarPersonajeSeleccionado() {
    const idSeleccionado = document.getElementById('selector-personaje').value;
    ...
    const guardados = JSON.parse(localStorage.getItem('dnd_personajes')) || [];
    const data = guardados.find(p => p.id === idSeleccionado);
    if (!data) return;
    ...
    modoJuego = data.progreso.modoJuego;
    puntosInicialesDisp = data.progreso.puntosInicialesDisp;
    nivelPersonaje = data.progreso.nivelPersonaje;
    puntosMejoraDisp = data.progreso.puntosMejoraDisp;
    hpActual = data.progreso.hpActual;
    manaActual = data.progreso.manaActual;
    ...
    if (modoJuego) {
        ...
        document.getElementById('fase-creacion').classList.remove('hidden');
        document.getElementById('fase-juego').classList.add('hidden');
        document.getElementById('col-asignar').classList.remove('hidden');
        document.getElementById('col-mejora').classList.add('hidden');
    }

    stats = data.statsArray;
    efectosPersonaje = data.efectosArray || [];
    ...
    eqIds.forEach((id, i) => {
        document.getElementById(id).value = eqData[i].val;
        document.getElementById(id).dataset.itemId = eqData[i].id;
    });

    data.inventario.mochila.forEach((item, i) => {
        document.getElementById(`inv-${i+1}`).value = item.val;
        document.getElementById(`inv-${i+1}`).dataset.itemId = item.id;
    });
...
function nuevoPersonaje() {
    ...
    document.getElementById('selector-personaje').value = "";
    
    document.getElementById('char-nombre').value = "";
    document.getElementById('char-img-url').value = "";
    document.getElementById('char-sexo').value = "";
    document.getElementById('char-edad').value = "";
    document.getElementById('char-clase').value = "";
    document.getElementById('char-historia').value = "";
    document.getElementById('char-img-display').src = "https://via.placeholder.com/150?text=Retrato";

    ['eq-armadura', 'eq-mano1', 'eq-mano2', 'inv-1', 'inv-2', 'inv-3', 'inv-4', 'inv-5'].forEach(id => {
        document.getElementById(id).value = "";
        document.getElementById(id).dataset.itemId = "";
    });

    modoJuego = false;
    puntosInicialesDisp = 15;
    nivelPersonaje = 1;
    puntosMejoraDisp = 0;
    ...
    hpActual = 20; hpMax = 20; manaActual = 20; manaMax = 20;
...
function borrarPersonaje() {
    ...
    if (confirm("¿Estás seguro de borrar este personaje permanentemente?")) {
        let guardados = JSON.parse(localStorage.getItem('dnd_personajes')) || [];
        guardados = guardados.filter(p => p.id !== idPersonajeActual);
        localStorage.setItem('dnd_personajes', JSON.stringify(guardados));
        nuevoPersonaje();
        cargarListaPersonajes();
    }
...
function calcularNivelFinal(stat) {
    let nivelBase = stat.id === 'vitalidad' ? stat.base + ((nivelPersonaje - 1) * stat.mult) : (stat.base + stat.ptsIniciales) + (stat.mult * stat.ptsMejora);
...
function recalcularMultiplicadores() {
    stats.forEach(stat => {
        if (stat.id !== 'vitalidad') {
            let n = stat.base + stat.ptsIniciales;
            stat.mult = n <= 2 ? 1 : n === 3 ? 2 : n <= 5 ? 3 : 4;
        }
    });
}

function renderizarEfectos() {
    const container = document.getElementById('contenedor-efectos');
    container.innerHTML = '';
...
function actualizarEquipamiento() {
    ...
    efectosPersonaje.forEach(efecto => {
        if (efecto.activo) { efecto.mods.forEach(mod => { const statObj = stats.find(s => s.id === mod.stat); if (statObj && !isNaN(mod.valor)) { statObj.bonusEfecto += mod.valor; } }); }
    });
    renderizarTabla();
}
...
function desequipar(idEquipado) {
    const inputEq = document.getElementById(idEquipado);
    if (!inputEq || !inputEq.value) return;
    let slotVacio = null;
    for (let i = 1; i <= 5; i++) { const invInput = document.getElementById(`inv-${i}`); if (invInput && !invInput.value) { slotVacio = invInput; break; } }
    if (!slotVacio) { alert("Mochila llena. No hay espacio para guardar esto."); return; }
    slotVacio.value = inputEq.value; slotVacio.dataset.itemId = inputEq.dataset.itemId || "";
    inputEq.value = ""; inputEq.dataset.itemId = "";
    actualizarEquipamiento();
}

function intercambiar(slotNum) {
    const invInput = document.getElementById(`inv-${slotNum}`);
    const targetInput = document.getElementById(document.getElementById(`target-${slotNum}`).value);
    if (!invInput || !targetInput) return;
    if (!invInput.value && !targetInput.value) return; 
    ...
    invInput.value = tempValorDestino; invInput.dataset.itemId = tempIdDestino;
    actualizarEquipamiento(); 
}
...
function modificarBarra(tipo, mult) {
    ...
    inputElem.value = 1; actualizarVisualesBarras();
    if(hpActual === 0) setTimeout(() => alert("¡TUS PUNTOS DE VIDA SON 0! Tira los dados."), 600);
}

function actualizarVisualesBarras() {
    const hpPct = (hpActual / hpMax) * 100;
    document.getElementById('hp-bar').style.width = `${hpPct}%`; document.getElementById('hp-text-display').innerText = `${hpActual} / ${hpMax}`;
    ...
    const manaPct = (manaActual / manaMax) * 100;
    document.getElementById('mana-bar').style.width = `${manaPct}%`; document.getElementById('mana-text-display').innerText = `${manaActual} / ${manaMax}`;
}
...
function activarAutocompletado(inputId, tiposPermitidos, afectaStats) {
    ...
    inp.addEventListener("input", function(e) {
        let val = this.value; cerrarListas(); 
        if (!val) { this.dataset.itemId = ""; if(afectaStats) actualizarEquipamiento(); return false; }
        ...
        coincidencias.forEach(item => {
            ...
            itemDiv.addEventListener("click", function(e) { inp.value = item.nombre; inp.dataset.itemId = item.id; cerrarListas(); if(afectaStats) actualizarEquipamiento(); });
...
function modificarPuntoInicial(i, v) {
    if (v === -1 && stats[i].ptsIniciales > 0) { stats[i].ptsIniciales -= 1; puntosInicialesDisp += 1; } 
    else if (v === 1 && puntosInicialesDisp > 0) { stats[i].ptsIniciales += 1; puntosInicialesDisp -= 1; }
    renderizarTabla();
}

function iniciarPartida() {
    modoJuego = true;
    document.getElementById('fase-creacion').classList.add('hidden'); document.getElementById('fase-juego').classList.remove('hidden');
    document.getElementById('col-asignar').classList.add('hidden'); document.getElementById('col-mejora').classList.remove('hidden');
    const vNivel = calcularNivelFinal(stats[0]); hpMax = vNivel.total; manaMax = vNivel.total; hpActual = hpMax; manaActual = manaMax;
    renderizarTabla();
}
