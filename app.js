...
let reglasMundo = null; // Guardará las reglas dictadas por el DM
...
// =========================================
// 1. SINCRONIZACIÓN CON EL DM (MODO DIOS)
// =========================================
async function cargarConfigMundo() {
    if (!historiaIdActual) return;
    const { data } = await window.db.from('historias').select('config_mundo').eq('id', historiaIdActual).single();
    
    if (data && data.config_mundo) {
        reglasMundo = data.config_mundo;
        
        // 1. Aplicar Color del Tema
        const barraTop = document.querySelector('.char-manager-bar');
        if(barraTop && reglasMundo.tema.primario) barraTop.style.backgroundColor = reglasMundo.tema.primario;

        // 2. Aplicar Restricciones de Inventario
        const inv = reglasMundo.inventario;
        if(document.getElementById('row-eq-armadura')) document.getElementById('row-eq-armadura').style.display = inv.pecho ? 'flex' : 'none';
        if(document.getElementById('row-eq-mano1')) document.getElementById('row-eq-mano1').style.display = inv.mano1 ? 'flex' : 'none';
        if(document.getElementById('row-eq-mano2')) document.getElementById('row-eq-mano2').style.display = inv.mano2 ? 'flex' : 'none';
        
        for(let i=1; i<=10; i++) {
            const slot = document.getElementById(`row-inv-${i}`);
            if(slot) slot.style.display = (i <= inv.mochila) ? 'flex' : 'none';
        }

        // 3. Inyectar Stats Customizados creados por el DM
        if (reglasMundo.stats_custom) {
            reglasMundo.stats_custom.forEach(customStat => {
                const existe = stats.find(s => s.id === customStat.id);
                if (!existe) {
                    stats.push({ id: customStat.id, nombre: customStat.nombre, base: 1, ptsIniciales: 0, mult: 1, ptsMejora: 0, bonusEq: 0, bonusEfecto: 0 });
                }
            });
        }
    }
...
// =========================================
// 2. RENDERIZADO MUTANTE DE LA TABLA
// =========================================
...
// =========================================
// EL RESTO DEL MOTOR (Ligeramente ajustado para la mochila infinita)
// =========================================
async function guardarPersonaje() {
    ...
    let mochilaArray = [];
    for(let i=1; i<=10; i++) { // Ampliado a 10 slots posibles
        const inp = document.getElementById(`inv-${i}`);
        if(inp) mochilaArray.push({ val: inp.value, id: inp.dataset.itemId || "" });
    }

    const payload = {
        user_id: user.id, historia_id: historiaIdActual, 
        ...
        stats_array: stats, efectos_array: efectosPersonaje,
        inventario: {
    ...
    if (!idPersonajeActual) {
        ...
        errorSupabase = error;
        if(data && data.length > 0) { idPersonajeActual = data[0].id; window.history.replaceState(null, '', `?id=${idPersonajeActual}`); }
    } else {
        const { error } = await window.db.from('personajes').update(payload).eq('id', idPersonajeActual);
        errorSupabase = error;
    }

    if (errorSupabase) { alert("Error al guardar."); console.error(errorSupabase); } 
    else { document.getElementById('nav-char-name').innerText = nombreStr; alert("¡Personaje Guardado!"); }
}
...
async function cargarPersonajeSeleccionado(idSeleccionado) {
    ...
    idPersonajeActual = data.id; historiaIdActual = data.historia_id;
    document.getElementById('nav-char-name').innerText = data.identidad.nombre;
    document.getElementById('char-nombre').value = data.identidad.nombre; document.getElementById('char-img-url').value = data.identidad.imgUrl; document.getElementById('char-sexo').value = data.identidad.sexo; document.getElementById('char-edad').value = data.identidad.edad; document.getElementById('char-clase').value = data.identidad.clase; document.getElementById('char-historia').value = data.identidad.historia;
    actualizarImagen();
    ...
    data.inventario.mochila.forEach((item, i) => { const inp = document.getElementById(`inv-${i+1}`); if(inp) { inp.value = item.val; inp.dataset.itemId = item.id; } });

    await cargarConfigMundo(); // CARGA LAS REGLAS DEL DM
    renderizarEfectos(); actualizarEquipamiento(); 
}

function nuevoPersonaje() {
    idPersonajeActual = null; document.getElementById('nav-char-name').innerText = "Personaje Nuevo";
    ...
    ['eq-armadura', 'eq-mano1', 'eq-mano2'].forEach(id => { document.getElementById(id).value = ""; document.getElementById(id).dataset.itemId = ""; });
    for(let i=1; i<=10; i++) { const inp = document.getElementById(`inv-${i}`); if(inp) { inp.value = ""; inp.dataset.itemId = ""; } }

    modoJuego = false; puntosInicialesDisp = 15; nivelPersonaje = 1; puntosMejoraDisp = 0; hpActual = 20; hpMax = 20; manaActual = 20; manaMax = 20;
    ...
    cargarConfigMundo().then(() => { renderizarEfectos(); actualizarEquipamiento(); });
}

// MATEMÁTICAS CLÁSICAS (Intactas)
...
function actualizarVisualesBarras() { const hpPct = (hpActual / hpMax) * 100; document.getElementById('hp-bar').style.width = `${hpPct}%`; document.getElementById('hp-text-display').innerText = `${hpActual} / ${hpMax}`; if (hpPct <= 30) { document.getElementById('hp-bar').classList.add('low'); document.getElementById('hp-text-display').classList.add('low'); } else { document.getElementById('hp-bar').classList.remove('low'); document.getElementById('hp-text-display').classList.remove('low'); } const manaPct = (manaActual / manaMax) * 100; document.getElementById('mana-bar').style.width = `${manaPct}%`; document.getElementById('mana-text-display').innerText = `${manaActual} / ${manaMax}`; }
async function borrarPersonaje() { if (!idPersonajeActual) return; if (confirm("¿Borrar personaje?")) { const { error } = await window.db.from('personajes').delete().eq('id', idPersonajeActual); if(!error) volverAlLobby(); else alert("Error al borrar en la nube."); } }
function volverAlLobby() { window.location.href = "index.html"; }

// Sistema de EFECTOS Y MAGIA
function renderizarEfectos() { const container = document.getElementById('contenedor-efectos'); container.innerHTML = ''; efectosPersonaje.forEach(efecto => { let modsHtml = ''; efecto.mods.forEach((mod, index) => { modsHtml += `<div class="mod-row"><select onchange="updateModStat(${efecto.id}, ${index}, this.value)" style="padding: 4px; border: 1px solid #bdc3c7; border-radius: 4px;">${stats.map(s => `<option value="${s.id}" ${mod.stat === s.id ? 'selected' : ''}>${s.nombre}</option>`).join('')}</select><input type="number" value="${mod.valor}" oninput="updateModValor(${efecto.id}, ${index}, this.value)" style="width: 50px; text-align: center; padding: 4px; border: 1px solid #bdc3c7; border-radius: 4px;"><button class="btn btn-icon btn-red" style="padding: 4px 8px;" onclick="eliminarMod(${efecto.id}, ${index})" title="Eliminar stat">x</button></div>`; }); container.innerHTML += `<div class="efecto-card"><div class="efecto-header"><input type="checkbox" ${efecto.activo ? 'checked' : ''} onchange="toggleEfecto(${efecto.id}, this.checked)" title="Activar"><input type="text" value="${efecto.nombre}" placeholder="Nombre..." oninput="updateEfectoNombre(${efecto.id}, this.value)"><button class="btn btn-icon btn-red" onclick="eliminarEfecto(${efecto.id})">Borrar</button></div>${modsHtml}<div style="padding-left: 30px; margin-top: 5px;"><button class="btn btn-small btn-blue" onclick="agregarMod(${efecto.id})">+ Añadir Stat</button></div></div>`; }); }
function agregarEfecto() { efectosPersonaje.push({ id: Date.now(), nombre: "Nuevo Efecto", activo: true, mods: [{ stat: 'vitalidad', valor: 1 }] }); renderizarEfectos(); actualizarEquipamiento(); }
...
async function cargarItemsDeLaCampana() { baseDatosItems = []; if (!historiaIdActual) return; const { data: itemsDM, error } = await window.db.from('items').select('*').eq('historia_id', historiaIdActual); if (error || !itemsDM) return; itemsDM.forEach(item => { baseDatosItems.push({ id: item.id, nombre: `✨ ${item.nombre}`, tipo: item.tipo, bonus: item.bonus }); }); }
...
function actualizarEquipamiento() {
    ...
    slotsEquipables.forEach(slot => { const input = document.getElementById(slot.id); if (input && input.dataset.itemId) { const item = baseDatosItems.find(i => i.id === input.dataset.itemId); if (item && item.bonus && slot.tiposValidos.includes(item.tipo)) { for (const [statId, valor] of Object.entries(item.bonus)) { const statObj = stats.find(s => s.id === statId); if (statObj) statObj.bonusEq += valor; } } } });
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (charId) {
        ...
        if (nuevaHistoriaId) { historiaIdActual = nuevaHistoriaId; document.getElementById('nav-char-name').innerText = "Héroe de Campaña"; }
        nuevoPersonaje();
    }
    ...
    for(let i=1; i<=10; i++) activarAutocompletado(`inv-${i}`, ["todos"], false);
});